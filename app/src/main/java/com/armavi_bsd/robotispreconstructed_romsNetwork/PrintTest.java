...
//    }
    ...
    private Bitmap convertToMonochrome(Bitmap original) {
        ...
        for (int i = 0; i < original.getWidth(); i++) {
            for (int j = 0; j < original.getHeight(); j++) {
                ...
                int monochromePixel = gray < 128 ? Color.BLACK : Color.WHITE; // Threshold for monochrome
                monochrome.setPixel(i, j, monochromePixel);
            }
        }
        return monochrome;
    }

    private byte[] convertBitmapToEscPosBytes(Bitmap bitmap) {
        ...
        // Loop through the image and convert to byte data
        int byteIndex = 0;
        for (int y = 0; y < height; y++) {
            for (int x = 0; x < width; x++) {
                // Get the pixel color
                int pixelColor = bitmap.getPixel(x, y);
                // If the pixel is black, mark it as 1, otherwise 0
                int bit = (pixelColor == Color.BLACK) ? 1 : 0;
                int byteOffset = (x / 8) + y * ((width + 7) / 8);
                if (bit == 1) {
                    imageBytes[byteOffset] |= (1 << (7 - (x % 8)));
                }
            }
        }
        return imageBytes;
    ...
    private void printLogo(OutputStream os) {
        // Retrieve the image from ImageView (you can replace this with your own image)
        Drawable drawable = dispCompanyIcon.getDrawable();
        Bitmap bitmap = ((BitmapDrawable) drawable).getBitmap();

        // Convert the image to monochrome
        Bitmap monochromeBitmap = convertToMonochrome(bitmap);

        // Convert the monochrome image to ESC/POS byte array
        byte[] imageData = convertBitmapToEscPosBytes(monochromeBitmap);

        // Print the image data to the printer
        try {
            os.write(imageData);
        } catch (IOException e) {
...
//            navigationState.navigateToActivity(getApplicationContext(), BluetoothDeviceList.class);
    ...
    //Function for printing receipt in POS printer
